...
export class DevisComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.devisService
      ...
      .subscribe(
        (res: HttpResponse<IDevis[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IDevis): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInDevis(): void {
    this.eventSubscriber = this.eventManager.subscribe('devisListModification', () => this.loadPage());
  }
