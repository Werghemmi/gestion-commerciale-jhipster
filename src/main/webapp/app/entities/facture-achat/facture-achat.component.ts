...
export class FactureAchatComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.factureAchatService
      ...
      .subscribe(
        (res: HttpResponse<IFactureAchat[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInFactureAchats(): void {
    this.eventSubscriber = this.eventManager.subscribe('factureAchatListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IFactureAchat[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/facture-achat'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
